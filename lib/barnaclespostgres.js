...
function handleSpatem(instance, spatem) {
  ...
  if(spatemOptions.filter.isPassing(spatem)) {
    ...
    if(Array.isArray(trimmedSpatem.data?.features)) {
      trimmedSpatem.data.features.splice(1); // Retain only device position
    }

    let text = 'INSERT INTO spatem (devicesignature, timestamp, spatem) ' +
               'VALUES ($1, $2, $3) ' +
               'RETURNING _storeId';
    let values = [ deviceSignature, new Date(trimmedSpatem.timestamp),
                   JSON.stringify(trimmedSpatem) ];
    ...
    instance.pool.query(text, values, (err, res) => {
      if(err) {
        ...
        let _storeId = res.rows[0]['_storeid'];
        let storedSpatem = Object.assign({ _storeId: _storeId }, trimmedSpatem);

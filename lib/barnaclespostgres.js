...
function handleRaddec(instance, raddec) {
  ...
  let text = 'INSERT INTO raddec (raddec) VALUES ($1) RETURNING _storeId';
  let values = [ JSON.stringify(flatRaddec) ];

  instance.client.query(text, values, (err, res) => {
    if(err && instance.printErrors) {
      console.log('barnacles-postgres:', err);
    }
    else {
...
function handleDynamb(instance, dynamb) {
  ...
  let text = 'INSERT INTO dynamb (dynamb) VALUES ($1) RETURNING _storeId';
  let values = [ JSON.stringify(dynamb) ];
